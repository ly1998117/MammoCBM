...
class CAMBase:
    ...
    def plot_slices(self, inp, heatmap, dirpath, filename, figsize=None, skip=1):
        import os
        import matplotlib.pyplot as plt
        import matplotlib.cm as cm
        if figsize is not None:
            inp = ((inp - inp.min()) / (inp.max() - inp.min())).cpu().detach()
            inp = torch.nn.functional.interpolate(inp, size=figsize, mode="trilinear",
                                                  align_corners=False).squeeze().numpy()

        heatmap = 1 - heatmap
        heatmap = heatmap / heatmap.max()
        heatmap = torch.nn.functional.interpolate(heatmap.cpu(), size=inp.shape[-3:],
                                                  mode="trilinear", align_corners=False)
        heatmap_colored = heatmap.squeeze().cpu().detach().numpy()
        heatmap_colored = cm.get_cmap("jet")(heatmap_colored)  # 生成彩
        # heatmap[heatmap < 128] = 0

        # 反转掩码：非蓝色区域为1，蓝色区域为0
        for i in range(0, inp.shape[-1], skip):
            inp_i = inp[..., i]
            heatmap_i = heatmap_colored[:, :, i, :]
            plt.figure(dpi=300)
            plt.subplot(2, 1, 1)
            plt.imshow(inp_i, cmap='gray')
            plt.title('Original Black and White Image')
            plt.axis('off')

            plt.subplot(2, 1, 2)
            plt.imshow(inp_i, cmap='gray', alpha=0.8)
            plt.imshow(heatmap_i, alpha=0.2)  # 热力图透明度为 0.3
            plt.title('Superimposed Heatmap')
            plt.axis('off')

            os.makedirs(dirpath, exist_ok=True)
            new_name = filename + f'_{i:03d}.png'
            plt.savefig(f'{dirpath}/{new_name.split(".")[0] + "heat.png"}')
            plt.close()
...
class GradCAM(CAMBase):
    ...
    def __call__(self, x, modality=None, class_idx=None, layer_idx=-1, retain_graph=False, **kwargs):
        """
        Compute the activation map with upsampling and postprocessing.
        ...
        return self._upsample_and_post_process(acti_map, x[modality])
